...
def matchToGenHadron(particle, event, minDR=0.05, minDpt=0.1):
    ...
    etacut = myeta + match[1]
    for i in range(ileft, len(event.packedGenForHadMatch)):
...
class ttHSVAnalyzer(Analyzer):
    def __init__(self, cfg_ana, cfg_comp, looperName):
        ...
        self.SVMVA = SVMVA(
            "%s/src/CMGTools/TTHAnalysis/data/btag/ivf/%%s_BDTG.weights.xml" % os.environ['CMSSW_BASE'])
        self.preselection = getattr(cfg_ana, 'preselection', lambda ivf: True)
        self.associatedJetsByRef = getattr(
            cfg_ana, 'associatedJetsByRef', True)
    ...
    def declareHandles(self):
        ...
        self.handles['ivf'] = AutoHandle(
            ('slimmedSecondaryVertices', ''), 'std::vector<reco::VertexCompositePtrCandidate>')
    ...
    def process(self, event):
        ...
        for sv in allivf:
            ...
            svtracks = []
            for id in range(sv.numberOfDaughters()):
                dau = sv.daughter(id)
                dau.sip3d = SignedImpactParameterComputer.signedIP3D(
                    dau.pseudoTrack(), pv, sv.momentum()).significance()
                svtracks.append(dau)
        ...
        event.ivf = list(filter(self.preselection, allivf))

        if self.cfg_comp.isMC and self.cfg_ana.do_mc_match:
            event.packedGenForHadMatch = [(p.eta(), p.phi(), p) for p in self.mchandles['packedGen'].product(
            ) if p.charge() != 0 and abs(p.eta()) < 2.7]
            event.packedGenForHadMatch.sort(key=lambda e_p_x: e_p_x[0])
            for s in event.ivf:
                        ...
                        while mom:
                            ...
                            if flav in [4, 5]:
                                ...
                                if key in ancestors:
                                    ancestors[key][1] += 1
                                    ancestors[key][2] = min(
                                        ancestors[key][2], depth)
                                else:
                                    ancestors[key] = [mom, 1, depth, flav]
                            mom = mom.mother(
                                0) if mom.numberOfMothers() > 0 else None
                            depth += 1
                        if found:
                            matchable += 1
                ...
                s.mcHadron = None
                if matchable:
                    maxhits = max([h for (p, h, d, f) in ancestors.values()])
                    mindepth = min(
                        [d for (p, h, d, f) in ancestors.values() if h == maxhits])
                    if matchable > 1:
                        s.mcMatchFraction = maxhits / float(matchable)
                    for (mom, hits, depth, flav) in ancestors.values():
                        if hits != maxhits:
                            continue
                        if depth == mindepth:
                            s.mcHadron = mom
        ...
        if self.associatedJetsByRef:
            ...
            for s in event.ivf:
                s.jet = None
                for i in range(s.numberOfDaughters()):
            ...
            for j in getattr(event, self.jetCollection):
                ...
                jdaus = [ref2id(j.daughterPtr(i))
                         for i in range(j.numberOfDaughters())]
                j.svs = []
                for jdau in jdaus:
                    if jdau in daumap:
        ...
        for l in event.selectedLeptons:
            ...
            for s in event.ivf:
                ...
                sip3d = SignedImpactParameterComputer.signedIP3D(
                    track.get(), s, s.momentum()).significance()
                byref = False
                daus = [ref2id(s.daughterPtr(i))
                        for i in range(s.numberOfDaughters())]
                for i in range(l.numberOfSourceCandidatePtrs()):
