...
if args.breakdown is not None:
    ...
    outputtxt = "val : {val:.19f}\n".format(val = val_nom[0])
    for i, br in enumerate(breakdown):
        ...
        outputtxt += "inc_{source} : {up:.19f} : {down:.19f}\n".format(source = br, up = hi, down = abs(lo))
    pt.AddText(textfit)

    print outputtxt
    with open("./outputfit.txt", "w") as thefile:
        thefile.write(outputtxt)
