...
class HadTopSimple:
    ...
    def __call__(self, event):
        ...
        for var in self.systsJEC:
            ...
            _ijets_list = getattr(
                event, "iJSel"+self.inputlabel+self.systsJEC[_var])
            ...
            for j1, j2, j3 in itertools.combinations(jets, 3):
                ...
                if abs(top_mass-172) < abs(best_top_mass-172):
                    ...
                    best_top_p4 = top_p4
                    best_top = sorted(
                        [j1, j2, j3], key=lambda x: x.btagCSV, reverse=True)
            ...
            if best_top:
                ...
                other_btags = [j.btagCSV for j in itertools.filterfalse(
                    lambda x: x in best_top, jets)]
                if len(other_btags) > 0:
                    ret["bJet_notFromHadTop_CSV"] = max(other_btags)
