...
class edgeFriends:
    def __init__(self, label, tightLeptonSel, cleanJet):
        ...
        self.puFile = open(
            "/afs/cern.ch/work/m/mdunser/public/puWeighting/puWeightsOfficialPrescription.txt", "r")
        self.pu_dict = eval(self.puFile.read())
        self.puFile.close()
        # filter things
        # =================
        self.beamHaloListFile = open(
            "/afs/cern.ch/user/p/pablom/public/Filters_27_01_2016/csc2015.txt", "r")
        self.fourthBadEESuperCrystalFile = open(
            "/afs/cern.ch/user/p/pablom/public/Filters_27_01_2016/ecalscn1043093.txt", "r")
        self.badResolutionTrackTaggerFile = open(
            "/afs/cern.ch/user/p/pablom/public/Filters_27_01_2016/badResolutionTrack.txt", "r")
        self.badMuonTrackTaggerFile = open(
            "/afs/cern.ch/user/p/pablom/public/Filters_27_01_2016/muonBadTrack.txt", "r")
        ...
        self.calib = ROOT.BTagCalibration(
            "csvv2", "/afs/cern.ch/user/p/pablom/public/CSVv2.csv")
        self.reader_heavy = ROOT.BTagCalibrationReader(
            self.calib, 1, "mujets", "central")
        self.reader_heavy_UP = ROOT.BTagCalibrationReader(
            self.calib, 1, "mujets", "up")
        self.reader_heavy_DN = ROOT.BTagCalibrationReader(
            self.calib, 1, "mujets", "down")
        self.reader_light = ROOT.BTagCalibrationReader(
            self.calib, 1, "comb", "central")
        self.reader_light_UP = ROOT.BTagCalibrationReader(
            self.calib, 1, "comb", "up")
        self.reader_light_DN = ROOT.BTagCalibrationReader(
            self.calib, 1, "comb", "down")

        self.calibFASTSIM = ROOT.BTagCalibration(
            "csvv2", "/afs/cern.ch/user/p/pablom/public/CSV_13TEV_Combined_20_11_2015.csv")
        self.reader_heavyFASTSIM = ROOT.BTagCalibrationReader(
            self.calibFASTSIM, 1, "fastsim", "central")
        self.reader_heavy_UPFASTSIM = ROOT.BTagCalibrationReader(
            self.calibFASTSIM, 1, "fastsim", "up")
        self.reader_heavy_DNFASTSIM = ROOT.BTagCalibrationReader(
            self.calibFASTSIM, 1, "fastsim", "down")
        self.reader_lightFASTSIM = ROOT.BTagCalibrationReader(
            self.calibFASTSIM, 1, "fastsim", "central")
        self.reader_light_UPFASTSIM = ROOT.BTagCalibrationReader(
            self.calibFASTSIM, 1, "fastsim", "up")
        self.reader_light_DNFASTSIM = ROOT.BTagCalibrationReader(
            self.calibFASTSIM, 1, "fastsim", "down")

        self.f_btag_eff = ROOT.TFile(
            "/afs/cern.ch/user/p/pablom/public/btageff__ttbar_powheg_pythia8_25ns.root")
        self.h_btag_eff_b = copy.deepcopy(
            self.f_btag_eff.Get("h2_BTaggingEff_csv_med_Eff_b"))
        self.h_btag_eff_c = copy.deepcopy(
            self.f_btag_eff.Get("h2_BTaggingEff_csv_med_Eff_c"))
        self.h_btag_eff_udsg = copy.deepcopy(
            self.f_btag_eff.Get("h2_BTaggingEff_csv_med_Eff_udsg"))
        self.f_btag_eff.Close()
        # =================
        # pdf things
        # =================
        self.an_file = ROOT.TFile(
            "/afs/cern.ch/work/m/mdunser/public/pdfsForLikelihood/pdfs_version24_savingTheWorkspace_june2016.root")
        ...
        for t in ['DA', 'MC']:
            for var in [['mlb', 'sum_mlb_Edge'], ['met', 'met_Edge'], ['zpt', 'lepsZPt_Edge'], ['ldr', 'lepsDR_Edge'], ['a3d', 'd3D_Edge'], ['ldp', 'lepsDPhi_Edge']]:
                print('loading likelihoods for variable %s in %s' %
                      (var[0], t))
                setattr(self, 'h_lh_ana_%s_%s' % (var[0], t), self.wspace.pdf(
                    '%s_analyticalPDF_%s' % (var[0], t)))
                setattr(self, 'var_ana_%s_%s' %
                        (var[0], t), self.wspace.var(var[1]))
                setattr(self, 'frame_ana_%s_%s' % (var[0], t), getattr(
                    self, 'var_ana_%s_%s' % (var[0], t)).frame())
                getattr(self, 'h_lh_ana_%s_%s' % (var[0], t)).plotOn(
                    getattr(self, 'frame_ana_%s_%s' % (var[0], t)))
                setattr(self, 'obs_ana%s_%s' %
                        (var[0], t), ROOT.RooArgSet(self.wspace.var(var[1])))
        # =================
        ...
        self.triggerlist = ['HLT_DoubleEl',  # ('HLT_Ele17_Ele12_CaloIdL_TrackIdL_IsoVL_DZ', 'HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ')
                            ...
                            'HLT_DoubleMu',
                            # ('HLT_Mu17_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL', 'HLT_Mu8_TrkIsoVVL_Ele17_CaloIdL_TrackIdL_IsoVL', 'HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL', 'HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL')
                            'HLT_MuEG',
                            # ('HLT_IsoMu24_eta2p1', 'HLT_IsoTkMu24_eta2p1', 'HLT_IsoMu18', 'HLT_IsoMu20', 'HLT_IsoTkMu20', 'HLT_IsoMu27', 'HLT_IsoTkMu27')
                            'HLT_SingleMu',
                            # ('HLT_Ele23_WPLoose_Gsf', 'HLT_Ele23_CaloIdL_TrackIdL_IsoVL', 'HLT_Ele27_WPLoose_Gsf', 'HLT_Ele27_eta2p1_WPLoose_Gsf', 'HLT_Ele32_eta2p1_WPLoose_Gsf', 'HLT_Ele27_WP85_Gsf', 'HLT_Ele27_eta2p    1_WP75_Gsf', 'HLT_Ele32_eta2p1_WP75_Gsf')
                            'HLT_SingleEl'
                            ]
    ...
    def listBranches(self):
        ...
        biglist = [("evt"+label, "D"),
                   ...
                   ("nLepTight"+label, "I"),
                   ("nLepLoose"+label, "I"),
                   ("nJetSel"+label, "I"), ("nJetSel_jecUp" +
                                            label, "I"), ("nJetSel_jecDn"+label, "I"),
                   ("bestMjj"+label, "F"),
                   ("minMjj"+label, "F"),
                   ("maxMjj"+label, "F"),
                   ("hardMjj"+label, "F"),
                   ...
                   # index >= 0 if in Jet; -1-index (<0) if in DiscJet
                   ("iJ"+label, "I", 20, "nJetSel"+label),
                   ("nLepGood20"+label, "I"),
                   ("nLepGood20T"+label, "I"),
                   ("nJet35"+label, "I"), ("nJet35_jecUp" + \
                                           label, "I"), ("nJet35_jecDn"+label, "I"),
                   ("htJet35j"+label, "F"), ("htJet35j_jecUp" + \
                                             label, "F"), ("htJet35j_jecDn"+label, "F"),
                   ("nBJetMedium25"+label, "I"), ("nBJetMedium25_jecUp" + \
                                                  label, "I"), ("nBJetMedium25_jecDn"+label, "I"),
                   ("nBJetLoose35"+label, "I"), ("nBJetLoose35_jecUp" + \
                                                 label, "I"), ("nBJetLoose35_jecDn"+label, "I"),
                   ("nBJetMedium35"+label, "I"), ("nBJetMedium35_jecUp" + \
                                                  label, "I"), ("nBJetMedium35_jecDn"+label, "I"),
                   ("iL1T"+label, "I"),
                   ("iL2T"+label, "I"),
                   ("lepsMll"+label, "F"),
                   ("lepsJZB"+label, "F"),
                   ...
                   ("metl1DPhi"+label, "F"),
                   ("metl2DPhi"+label, "F"),
                   ("met"+label, "F"), ("met_phi"+label,
                                        "F"), ("met_jecUp"+label, "F"), ("met_jecDn"+label, "F"),
                   ("lepsDPhi"+label, "F"),
                   ("Lep1_pt"+label, "F"),
                   ("Lep1_eta"+label, "F"),
                   ("Lep1_phi"+label, "F"),
                   ...
                   ("st"+label, "F"),
                   ("srID"+label, "I"),
                   ("mt2"+label, "F"), ("mt2_jecUp" + \
                                        label, "F"), ("mt2_jecDn"+label, "F"),
                   ...
                   ("nll"+label, "F"), ("nll_jecUp" + \
                                        label, "F"), ("nll_jecDn"+label, "F"),
                   ("nll_mc"+label, "F"), ("nll_mc_jecUp" + \
                                           label, "F"), ("nll_mc_jecDn"+label, "F"),
        ...
        for jfloat in "pt eta phi mass btagCSV rawPt".split():
            biglist.append(("JetSel"+label+"_"+jfloat,
                           "F", 20, "nJetSel"+label))
        # if self.isMC:
    ...
    def __call__(self, event):
        ...
        jetsd_jecUp = [j for j in Collection(
            event, "DiscJet_jecUp", "nDiscJet_jecUp")]
        jetsc_jecDn = [j for j in Collection(
            event, "Jet_jecDown", "nJet_jecDown")]
        jetsd_jecDn = [j for j in Collection(
            event, "DiscJet_jecDown", "nDiscJet_jecDown")]
        ...
        for mass in self.susymasslist:
            ret[mass] = (-1 if not hasattr(event, mass)
                         else getattr(event, mass))
        for trig in self.triggerlist:
            if not isData:
                trigret[trig] = -1
            else:
                trigret[trig] = (-1 if not hasattr(event, trig)
                                 else getattr(event, trig))
        t1 = time.time()

        ret['genWeight'] = (1. if not hasattr(
            event, 'genWeight') else getattr(event, 'genWeight'))
        ret['hbheFilterIso'] = (1 if not hasattr(
            event, 'hbheFilterIso') else int(getattr(event, 'hbheFilterIso')))
        ret['hbheFilterNew25ns'] = (1 if not hasattr(
            event, 'hbheFilterNew25ns') else int(getattr(event, 'hbheFilterNew25ns')))
        ret['Flag_eeBadScFilter'] = (1 if not hasattr(
            event, 'Flag_eeBadScFilter') else int(getattr(event, 'Flag_eeBadScFilter')))
        ...
        ret["iLT"].sort(key=lambda idx: leps[idx].pt if idx >=
                        0 else lepso[-1-idx].pt, reverse=True)
        t23 = time.time()
        ...
        t24 = time.time()
        ret['iL1T'] = ret["iLT"][iL1iL2[0]] if (
            len(ret["iLT"]) >= 1 and iL1iL2[0] != -999) else -999
        ret['iL2T'] = ret["iLT"][iL1iL2[1]] if (
            len(ret["iLT"]) >= 2 and iL1iL2[1] != -999) else -999
                ...
                if not isData:
                    for tau in gentaus:
                        ...
                        if tmp_dr < minDRTau:
                            minDRTau = tmp_dr
                for lfloat in 'pt eta phi miniRelIso pdgId mvaIdSpring15'.split():
                    lepret["Lep"+str(lcount)+"_"+lfloat +
                           self.label] = getattr(lep, lfloat)
                lepvectors.append(lep)
                lepret['metl'+str(lcount)+'DPhi' +
                       self.label] = abs(deltaPhi(getattr(lep, 'phi'), metphi))
                lepret["Lep"+str(lcount)+"_"+"minTauDR"+self.label] = minDRTau
                ltlvs[lcount-1].SetPtEtaPhiM(lep.pt, lep.eta,
                                             lep.phi, 0.0005 if lep.pdgId == 11 else 0.106)
                lcount += 1
        ...
        mt2_jecDn = -1.
        if ret['nPairLep'] == 2:
            l1mt2 = ROOT.reco.Particle.LorentzVector(lepvectors[0].p4().Px(
            ), lepvectors[0].p4().Py(), lepvectors[0].p4().Pz(), lepvectors[0].p4().Energy())
            l2mt2 = ROOT.reco.Particle.LorentzVector(lepvectors[1].p4().Px(
            ), lepvectors[1].p4().Py(), lepvectors[1].p4().Pz(), lepvectors[1].p4().Energy())
            metp4obj = ROOT.reco.Particle.LorentzVector(
                met*cos(metphi), met*sin(metphi), 0, met)
            metp4obj_jecUp = ROOT.reco.Particle.LorentzVector(
                ret['met_jecUp']*cos(metphi), ret['met_jecUp']*sin(metphi), 0, ret['met_jecUp'])
            metp4obj_jecDn = ROOT.reco.Particle.LorentzVector(
                ret['met_jecDn']*cos(metphi), ret['met_jecDn']*sin(metphi), 0, ret['met_jecDn'])
        ...
        jetsd = self.setJetCollection(jetsd, lepst)
        jetsc_jecUp = self.setJetCollection(jetsc_jecUp, lepst)
        jetsd_jecUp = self.setJetCollection(jetsd_jecUp, lepst)
        jetsc_jecDn = self.setJetCollection(jetsc_jecDn, lepst)
        jetsd_jecDn = self.setJetCollection(jetsd_jecDn, lepst)

        (ret["iJ"], nb25, nb35, nl35, n35, ht35, theJets,
         theBJets) = self.countJets(jetsc, jetsd)
        (ijlist_jecup, nb25_jecUp, nb35_jecUp, nl35_jecUp, n35_jecUp, ht35_jecUp,
         theJets_jecUp, theBJets_jecUp) = self.countJets(jetsc_jecUp, jetsd_jecUp)
        (ijlist_jecdn, nb25_jecDn, nb35_jecDn, nl35_jecDn, n35_jecDn, ht35_jecDn,
         theJets_jecDn, theBJets_jecDn) = self.countJets(jetsc_jecDn, jetsd_jecDn)

        ret['nJet35'] = n35
        ret['nJet35_jecUp'] = n35_jecUp
        ret['nJet35_jecDn'] = n35_jecDn
        ret['nBJetMedium25'] = nb25
        ret['nBJetMedium25_jecUp'] = nb25_jecUp
        ret['nBJetMedium25_jecDn'] = nb25_jecDn
        ret['nBJetMedium35'] = nb35
        ret['nBJetMedium35_jecUp'] = nb35_jecUp
        ret['nBJetMedium35_jecDn'] = nb35_jecDn
        ret['nBJetLoose35'] = nl35
        ret['nBJetLoose35_jecUp'] = nl35_jecUp
        ret['nBJetLoose35_jecDn'] = nl35_jecDn
        ret["htJet35j"] = ht35
        ret["htJet35j_jecUp"] = ht35_jecUp
        ret["htJet35j_jecDn"] = ht35_jecDn
        ...
        ret["iJ"].sort(key=lambda idx: jetsc[idx].pt if idx >=
                       0 else jetsd[-1-idx].pt, reverse=True)
        ...
        for idx in ret["iJ"]:
            ...
            if not isData:
                for jmc in "mcPt mcFlavour mcMatchId".split():
                    jetret[jmc].append(
                        getattr(jet, jmc) if not isData else -1.)
        t6 = time.time()
        ...
        ret['j1MetDPhi'] = deltaPhi(
            metphi, theJets[0].phi) if len(theJets) > 0 else -99.
        ret['j2MetDPhi'] = deltaPhi(
            metphi, theJets[1].phi) if len(theJets) > 1 else -99.

        t7 = time.time()
        [wtbtag, wtbtagUp_heavy, wtbtagUp_light, wtbtagDown_heavy, wtbtagDown_light] = (
            self.getWeightBtag(theJets) if not isData else [1., 1., 1., 1., 1.])
            ...
            for _il, lep in enumerate(leplist):
                ...
                for _ij, j in enumerate(jet2coll):
                    if len(theBJets) == 1 and j.btagCSV >= self.btagMediumCut:
                        continue
                    if (len(theBJets) == 0 or len(theBJets) >= 2) and _ij == _jmin:
                        continue
                    jet.SetPtEtaPhiM(j.pt, j.eta, j.phi, j.mass)
                    tmp_mlb = (lep+jet).M()
                    if tmp_mlb < max_mlb:
        ...
        ret["sum_mlb"] = (ret["min_mlb1"] + ret["min_mlb2"]
                          ) if ret["min_mlb1"] > 0. and ret["min_mlb2"] > 0. else -1.
        ret["st"] = met+lepret["Lep1_pt"+self.label] + \
            lepret["Lep2_pt"+self.label]
        t9 = time.time()
        ...
        isBasicSREvent = (ret['nPairLep'] > 0 and ret["lepsDR"] > 0.1 and lepret["Lep1_pt" +
                          self.label] > 20. and lepret["Lep2_pt"+self.label] > 20. and ret['lepsMll'] > 20.)
        isBasicSREvent = isBasicSREvent * \
            (abs(lepret["Lep1_eta"+self.label] - 1.5) >
             0.1 and abs(lepret["Lep2_eta"+self.label] - 1.5) > 0.1)
        isBasicSREvent = isBasicSREvent * \
            (met > 150 and ret['nJetSel'] >= 2)  # adapting to 2016 selection
        if isBasicSREvent:
            srID = self.getSRID(ret['lepsMll'], lepret["Lep1_eta"+self.label],
                                lepret["Lep2_eta"+self.label], ret["nBJetMedium35"])
            ret["srID"] = srID
            for t in ['data', 'mc']:
                if t == 'data':
                    nam = 'DA'
                ...
                for u in ['_ana']:
                    for var in [['mlb', ret['sum_mlb'], 'sum_mlb_Edge'], ['met', met, 'met_Edge'],
                                ['zpt', ret['lepsZPt'], 'lepsZPt_Edge'], [
                                    'ldr', ret['lepsDR'], 'lepsDR_Edge'],
                                ['a3d', ret['d3D'], 'd3D_Edge'], ['ldp', ret['lepsDPhi'], 'lepsDPhi_Edge']]:
                        self.wspace.var(var[2]).setVal(var[1])
                        ret["lh%s_%s_%s" % (u, var[0], t)] = getattr(self, 'h_lh_ana_%s_%s' % (
                            var[0], nam)).getVal(getattr(self, 'obs_ana%s_%s' % (var[0], nam)))

                    if not ret["lh%s_mlb_%s" % (u, t)]:
                        ret["lh%s_mlb_%s" % (u, t)] = 1e-50
                    if not ret["lh%s_ldr_%s" % (u, t)]:
                        ret["lh%s_ldr_%s" % (u, t)] = 1e-50
                    if not ret["lh%s_met_%s" % (u, t)]:
                        ret["lh%s_met_%s" % (u, t)] = 1e-50
                    if not ret["lh%s_zpt_%s" % (u, t)]:
                        ret["lh%s_zpt_%s" % (u, t)] = 1e-50
                    if not ret["lh%s_a3d_%s" % (u, t)]:
                        ret["lh%s_a3d_%s" % (u, t)] = 1e-50
                    if not ret["lh%s_ldp_%s" % (u, t)]:
                        ret["lh%s_ldp_%s" % (u, t)] = 1e-50
            ret['nll'] = -1.*math.log(ret["lh_ana_mlb_data"]*ret["lh_ana_met_data"]
                                      * ret["lh_ana_zpt_data"]*ret["lh_ana_ldp_data"])
            ret['nll_mc'] = -1.*math.log(ret["lh_ana_mlb_mc"] * ret["lh_ana_met_mc"]
                                         * ret["lh_ana_zpt_mc"] * ret["lh_ana_ldp_mc"])
        else:
            ret["srID"] = -99
            for t in ['data', 'mc']:
                for u in ['_ana']:
    ...
    def getParOrtPt(self, l1, l2):
        ...
        # projection of the softer lepton onto the harder
        p1 = math.cos(v1.Angle(v2)) * v2.Mag() * u1
        # orthogonal to the projection of the softer onto the harder
        o1 = v1 - p1
        return (p1.Perp(), o1.Perp())

    def getPairVariables(self, lepst, metp4, metp4_raw):
        ret = (-999, -999, -99., -9000., -9000, -99., -
               99., -99., -99., -99., -99., -99., -99., -99.)
        if len(lepst) >= 2:
            [mll, jzb, jzb_raw, dr, metrec, zpt, dphi, d3D] = self.getMll_JZB(
                lepst[0].p4(), lepst[1].p4(), metp4, metp4_raw)
            [parPt, ortPt] = self.getParOrtPt(lepst[0].p4(), lepst[1].p4())
            ret = (0, 1, mll, jzb, jzb_raw, dr, metrec, zpt, dphi, d3D,
                   parPt, ortPt, lepst[0].p4().Theta() - lepst[1].p4().Theta())
        return ret
    ...
    def getHardMjj(self, jetsel, _dphi=False, _dr=False):
        ...
        if not _dphi:
            jet1 = ROOT.TLorentzVector()
            jet2 = ROOT.TLorentzVector()
            jet1.SetPtEtaPhiM(jetsel[0].pt, jetsel[0].eta,
                              jetsel[0].phi, jetsel[0].mass)
            jet2.SetPtEtaPhiM(jetsel[1].pt, jetsel[1].eta,
                              jetsel[1].phi, jetsel[1].mass)
            retval = (jet1+jet2).M()
        else:
            if not _dr:
                retval = deltaPhi(jetsel[0].phi, jetsel[1].phi)
            else:
                retval = deltaR(jetsel[0], jetsel[1])
        return retval
    ...
    def get_SF_btag(self, pt, eta, mcFlavour):
        ...
        if flavour == 2:
            ...
            SFcorr = self.reader_lightFASTSIM.eval(
                flavour, eta_cutoff, pt_cutoff)
            SFupcorr = self.reader_light_UPFASTSIM.eval(
                flavour, eta_cutoff, pt_cutoff)
            SFdowncorr = self.reader_light_DNFASTSIM.eval(
                flavour, eta_cutoff, pt_cutoff)
        else:
            ...
            SFcorr = self.reader_heavyFASTSIM.eval(
                flavour, eta_cutoff, pt_cutoff)
            SFupcorr = self.reader_heavy_UPFASTSIM.eval(
                flavour, eta_cutoff, pt_cutoff)
            SFdowncorr = self.reader_heavy_DNFASTSIM.eval(
                flavour, eta_cutoff, pt_cutoff)
    ...
    def getWeightBtag(self, jets):
        ...
        for jet in jets:
            ...
            csv = jet.btagCSV
            mcFlavor = (jet.hadronFlavour if hasattr(
                jet, 'hadronFlavour') else jet.mcFlavour)
            eta = jet.eta
            pt = jet.pt
    ...
    def _susyEdgeLoose(self, lep):
        ...
        if abs(lep.pdgId) == 11:
            if lepeta > 2.5:
                return False
            if (lep.convVeto == 0) or (lep.lostHits > 0):
                return False
            if (lepeta < 0.8 and lep.mvaIdSpring15 < -0.70):
                return False
            if (lepeta > 0.8 and lepeta < 1.479 and lep.mvaIdSpring15 < -0.83):
                return False
            if (lepeta > 1.479 and lep.mvaIdSpring15 < -0.92):
                return False
...
def _susyEdgeTight(lep):
    ...
    if abs(lep.dz) > 0.10:
        return False
    if eta > 1.4 and eta < 1.6:
        return False
    if abs(lep.pdgId) == 13:
        if lep.mediumMuonId != 1:
            return False
        if lep.miniRelIso > 0.2:
            return False
    ...
    if abs(lep.pdgId) == 11:
        ...
        if (etatest > 1.4442 and etatest < 1.566):
            return False
        if (lep.convVeto == 0) or (lep.lostHits > 0):
            return False
        if (eta < 0.8 and lep.mvaIdSpring15 < 0.87):
            return False
        if (eta > 0.8 and eta < 1.479 and lep.mvaIdSpring15 < 0.60):
            return False
        if (eta > 1.479 and lep.mvaIdSpring15 < 0.17):
            return False
        if lep.miniRelIso > 0.1:
            return False
    return True
...
if __name__ == '__main__':
    ...
    class Tester(Module):
        ...
        def analyze(self, ev):
            print("\nrun %6d lumi %4d event %d: leps %d" %
                  (ev.run, ev.lumi, ev.evt, ev.nLepGood))
            print(self.sf1(ev))
            print(self.sf2(ev))
            print(self.sf3(ev))
            print(self.sf4(ev))
