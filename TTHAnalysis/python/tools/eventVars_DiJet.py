...
# https://github.com/HephySusySW/Workspace/blob/72X-master/RA4Analysis/python/mt2w.py
def getPhysObjectArray(j):
    px = j.pt*cos(j.phi)
    py = j.pt*sin(j.phi)
    pz = j.pt*sinh(j.eta)
...
class EventVarsDiJet:
    ...
    def __call__(self, event, keyvals):
        ...
        metp4.SetPtEtaPhiM(event.met_pt, event.met_eta,
                           event.met_phi, event.met_mass)
       # njet = len(jets);
        nlep = len(leps)
        ...
        if nlep > 0:
            ...
            if event.isData == False:
                ...
                if ngenlep > 0:
                    minL1g = min([deltaR(g, leps[0])
                                 for g in genleps]) if nlep > 0 else 99
                    minL2g = min([deltaR(g, leps[1])
                                 for g in genleps]) if nlep > 1 else 99
                    glep1pt = -99
                    glep2pt = -99
                    glep1eta = -99
                    glep2eta = -99
                    ...
                    ret["mindr_lep1_gen"] = deltaR(
                        genleps[0], leps[0]) if nlep > 0 else 99
                    ret["mindr_lep2_gen"] = deltaR(
                        genleps[1], leps[1]) if nlep > 1 and ngenlep > 1 else 99
        ...
        if njet > 0:
            ...
            if event.isData == False:
                ...
                if abs(jets[0].mcFlavour) == 5:
                    J1_Gb = 1
                if njet > 1 and abs(jets[1].mcFlavour) == 5:
                    J2_Gb = 1
                if njet > 2 and abs(jets[2].mcFlavour) == 5:
                    J3_Gb = 1

                if abs(jets[0].mcFlavour) == 4:
                    J1_Gc = 1
                if njet > 1 and abs(jets[1].mcFlavour) == 4:
                    J2_Gc = 1
                if njet > 2 and abs(jets[2].mcFlavour) == 4:
                    J3_Gc = 1
            ...
            if njet > 1:
                J2_btag = jets[1].btagCSV
            if njet > 2:
                J3_btag = jets[2].btagCSV
            ...
            if jets[0].btagCSV > 0.890:
                J1_bJet = 1
            if njet > 1 and jets[1].btagCSV > 0.890:
                J2_bJet = 1
            if njet > 2 and jets[2].btagCSV > 0.890:
                J3_bJet = 1
            if jets[0].btagCSV > 0.605:
                J1_bJetL = 1
            if njet > 1 and jets[1].btagCSV > 0.605:
                J2_bJetL = 1
            if njet > 2 and jets[2].btagCSV > 0.605:
                J3_bJetL = 1
#        print "\nrun %6d lumi %4d event %d: btag %f" % (event.run, event.lumi, event.evt, jets[0].btagCSV)

            ret["mindr_lep_jet1"] = (
                deltaR(jets[0], leps[0])) if nlep >= 1 else -99
            ret["mindr_lep_jet2"] = (
                deltaR(jets[1], leps[0])) if nlep >= 1 and njet > 1 else -99
            ret["mindr_lep_jet3"] = (
                deltaR(jets[2], leps[0])) if nlep >= 1 and njet > 2 else -99
            ret["mindr_lep_jet4"] = (
                deltaR(jets[3], leps[0])) if nlep >= 1 and njet > 3 else -99
            ret['J1_pT'] = JC1_pT
            ret['J2_pT'] = JC2_pT
            ret['J3_pT'] = JC3_pT
            ret['J4_pT'] = JC4_pT
        ...
        if njet > 1:
            ...
            MCT_all = sqrt(2*jets[0].pt*jets[1].pt *
                           (1+cos(jets[0].phi-jets[1].phi)))
            if jets[0].btagCSV > bTagWP and jets[1].btagCSV > bTagWP:
                MCT_2b = sqrt(2*jets[0].pt*jets[1].pt *
                              (1+cos(jets[0].phi-jets[1].phi)))
            ...
            if njet > 2:
                J2J3 = (jets[1].p4()+jets[2].p4())
            if njet > 2:
                J2J3M = (jets[1].p4()+jets[2].p4()).M()
            if njet > 2:
                MCT_J1 = sqrt(2*jets[1].pt*jets[2].pt *
                              (1+cos(jets[1].phi-jets[2].phi)))
            ...
            for n, j in enumerate(jets):
                ...
                if thisDeltaPhi < deltaPhiMin_had3:
                    deltaPhiMin_had3 = thisDeltaPhi
