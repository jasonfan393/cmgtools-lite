...
class ttHLepMEMFriend:
    ...
    def __call__(self,event):
        ...
        for jecPostfix in self.JECs:
            ...
            ljets = [j for j in jets if j not in bjets]
            for j in bjets: 
            ...
            if len(bjets) >= 1:
                ...
                if len(ljets)>=2:
                    ...
                    pairs = [(ljets[i1],ljets[i2],(ljets[i1].p4()+ljets[i2].p4()).M()) for i1 in range(len(ljets)-1) for i2 in range(i1+1,len(ljets)) ]
                    def fillTwoPairs(pairs):
                        print('choosing among %d pairs'%len(pairs))
                        if len(pairs)==0: return
                        pairs.sort(key = lambda j1_j2_m : abs(j1_j2_m[2]-80.419))
                        ...
                        pairs.sort(key = lambda j1_j2_m1 : j1_j2_m1[2])
...
if __name__ == '__main__':
    ...
    class Tester(Module):
        def __init__(self, name):
            ...
            if len(argv) > 4: self.sf.selectEvents( list(map(int,argv[4:])) )
        def analyze(self,ev):
            self.sf(ev)
